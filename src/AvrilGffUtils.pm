...
sub make_gff_for_features_in_subsequences
{
	...
	$line                = $_;
	chomp $line;
	# eg. scaff1  sourcefeature2030.+.feature1
	@temp                = split(/\t+/,$line);
	...
	$line                = $_;
	chomp $line;
	# eg. scaff1sourcefeature1535.+.feature1
	@temp                = split(/\t+/,$line);
	$scaffold            = $temp[0];
...
sub add_gene_features_to_gff_for_subsequences
{
	...
	@temp                = split(/\s+/,$line);
	# eg.
	# SSTP.contig.00005.803316sourceCDS83409119.+.T1.SSTP.contig.00005.803316_1_1;Parent=T1.SSTP.contig.00005.803316_1_mRNA
	# SSTP.contig.00005.803316sourcegene81719119.+.T1.SSTP.contig.00005.803316_1
	# SSTP.contig.00005.803316sourcemRNA81719119.+.T1.SSTP.contig.00005.803316_1_mRNA;Parent=T1.SSTP.contig.00005.803316_1
	$scaffold            = $temp[0];
	$feature_type        = $temp[2];
...
sub read_genenames_for_transcripts
{
	...
	if (substr($line,0,1) ne '#' && $end_of_gff == 0)
	{
		...
		# eg. SSTP.contig.00005.803316sourcemRNA81719119.+.T1.SSTP.contig.00005.803316_1_mRNA;Parent=T1.SSTP.contig.00005.803316_1
		$scaffold      = $temp[0];
		$gene          = $temp[8];
